...
function recordatorioCS_form() {
		?>
		<form method="post">
			<p><label for="texAlarma"><?php esc_html_e('Introduzca nombre del Evento', 'recordatorioCS');?></label></p>
	...
	function recordatorioCS_process() {
		if ( isset($_POST['texto-alarma'])) {
			...
			//	'<p>El evento '.$txtAlarma.' para '.$fecha.' y '.$hora.' horas ha sido enviado al correo '.$to.'</p>';
				  ...
				  esc_html_e('El nombre del evento esta vacio', 'recordatorioCS');
			}
	...
	function recordatorioCS_display_settings_page() {
